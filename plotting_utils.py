...
def plot_eigenvalues(axes, eigval1, eigval2, color='black', label=['',''], alpha=1):
    axes.scatter(np.real(eigval1), np.imag(eigval1), color=color, label=label[0], marker='o', alpha=alpha)
    axes.scatter(np.real(eigval2), np.imag(eigval2), color=color, label=label[1], marker='s', alpha=alpha)
